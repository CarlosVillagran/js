...
object ArResources {
    fun init(context: Context, selection: String): CompletableFuture<Void> {
        when (selection) {
            "buho.jpg" -> {
                swarmRendereable = ModelRenderable.builder().setSource(context, Uri.parse("buho3d.sfb")).build()
        ...
        when (selection) {
            ...
            "elefante.jpg" -> {
                videoPlayer = MediaPlayer.create(context, R.raw.elefante)
            }
            "mexico.jpg" -> {
                videoPlayer = MediaPlayer.create(context, R.raw.mexico)
            }
            "mujer.jpg" -> {
                videoPlayer = MediaPlayer.create(context, R.raw.mujer)
            }
            "conejo.jpg" -> {
                videoPlayer = MediaPlayer.create(context, R.raw.conejo)

            }
            "salem.jpg" -> {
                videoPlayer = MediaPlayer.create(context, R.raw.salem)
            }
